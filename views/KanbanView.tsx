...
export const KanbanView: React.FC<KanbanViewProps> = ({
    ...
    return (
        <div className="h-full flex flex-col min-w-0 min-h-0">
            {/* Quick Filter Bar */}
            <div className="px-6 pb-2 pt-2 flex items-center justify-between border-b border-marga-creamDark bg-white">
                <div className="flex gap-2 bg-gray-100 p-1 rounded-lg">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-md text-xs font-bold transition-all ${filterMode === 'AUTO'
                            ? 'bg-white text-marga-wine shadow-sm ring-1 ring-black/5'
                            : 'text-gray-500 hover:text-marga-wine hover:bg-marga-creamDark/50'
                            }`}
                    >
                        <Bot size={14} />
                        Modo: AUTO
                        <span className="bg-gray-100 text-gray-500 px-1.5 rounded-full text-[10px] border border-marga-creamDark ml-1">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-md text-xs font-bold transition-all ${filterMode === 'HUMANO'
                            ? 'bg-white text-marga-cream shadow-sm ring-1 ring-black/5'
                            : 'text-gray-500 hover:text-marga-wine hover:bg-marga-creamDark/50'
                            }`}
                    >
                        <User size={14} />
            ...
            <div className="flex-1 overflow-x-auto overflow-y-hidden pt-4 bg-marga-cream/50">
                            ...
                            <div key={col.id} className="w-80 flex flex-col h-full group">
                                ...
                                <div className="mb-4 px-1">
                                    <div className="flex items-center justify-between mb-1">
                                        ...
                                        <span className="bg-white border border-marga-creamDark text-gray-600 text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">
                                            {colLeads.length}
                                        </span>
                                    </div>
                                ...
                                {/* Drop Zone */}
                                <div className="flex-1 bg-gray-100 rounded-xl p-2 overflow-y-auto border border-marga-creamDark shadow-inner">
                                    {colLeads.map((lead) => (
                                        <LeadCard
                                            key={lead.id}
                                            lead={lead}
                                    ...
                                    {colLeads.length === 0 && (
                                        <div className="h-24 flex items-center justify-center border-2 border-dashed border-marga-creamDark rounded-xl m-2 opacity-50">
